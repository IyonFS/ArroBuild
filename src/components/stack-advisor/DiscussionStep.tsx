...
export default function DiscussionStep({
  ...
  const remaining = STACK_ADVISOR_DISCUSSION_MAX_TURNS - turnCount;
  const canSend =
    draft.trim() && !loading && turnCount < STACK_ADVISOR_DISCUSSION_MAX_TURNS;
  ...
  const send = async () => {
    ...
    setDraft("");
    const nextMessages: StackDiscussMessage[] = [
      ...messages,
      { role: "user", content: userMessage },
    ];

    try {
      const res = await fetch("/api/tools/stack-advisor/discuss", {
        method: "POST",
    ...
    try {
      const res = await fetch("/api/tools/stack-advisor/discuss", {
        ...
        body: JSON.stringify({
          filledFields: {},
          messages: [],
          userMessage: "Halo, aku bingung pilih stack untuk proyekku.",
          turnCount: 0,
        }),
      });
      const data = await res.json();
      ...
      onUpdate({
        messages: [
          { role: "user", content: "Halo, aku bingung pilih stack untuk proyekku." },
          { role: "assistant", content: data.assistantMessage },
        ],
  ...
  const ready =
    isComplete ||
    Boolean(
      filledFields.productType &&
        filledFields.priority &&
        (filledFields.mainProblem || filledFields.targetUser || filledFields.notes)
    );
  ...
  return (
    <div className="mx-auto max-w-2xl">
      ...
      >
        Hard cap {STACK_ADVISOR_DISCUSSION_MAX_TURNS} giliran — pola sama Mode Dipandu AI.
      </p>
      ...
      >
        ...
        <span className="font-mono text-sm font-bold" style={{ color: "var(--app-sky)" }}>
          {turnCount}/{STACK_ADVISOR_DISCUSSION_MAX_TURNS}
      ...
      {Object.keys(filledFields).length > 0 && (
        ...
        >
          ...
          {filledFields.productType && <div>Tipe: {filledFields.productType}</div>}
          {filledFields.priority && <div>Prioritas: {filledFields.priority}</div>}
          {filledFields.stage && <div>Stage: {filledFields.stage}</div>}
          {filledFields.targetUser && <div>Target: {filledFields.targetUser}</div>}
          {filledFields.mainProblem && <div>Problem: {filledFields.mainProblem}</div>}
        </div>
      )}
        ...
        {messages.length === 0 ? (
          <div className="flex flex-1 flex-col items-center justify-center gap-3 py-8">
            <p className="font-mono text-sm" style={{ color: "var(--color-text-secondary)" }}>
              Mulai curhat — kami gali brief-nya.
            </p>
            <button
              type="button"
      ...
      <div className="mb-6 flex gap-2">
        <input
          ...
          disabled={loading || turnCount >= STACK_ADVISOR_DISCUSSION_MAX_TURNS}
